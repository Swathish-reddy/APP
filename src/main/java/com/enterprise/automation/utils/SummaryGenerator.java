...
public class SummaryGenerator {
    ...
    public static void generateMarkdownSummary(ITestContext context) {
        ...
        double failPct = (double) failed / total * 100;
        ...
        sb.append("- **Fail Percentage:** ").append(String.format("%.2f", failPct)).append("%\n\n");
        
        sb.append("## Valid Test Case Summary (Sample)\n\n");
        
        sb.append("### FAILED TESTS\n\n");
        context.getFailedTests().getAllResults().stream().limit(10).forEach(res -> {
            sb.append("✗ ").append(res.getMethod().getMethodName()).append("\n");
        ...
        // Also append to GitHub Actions GITHUB_STEP_SUMMARY if environment variable exists
