...
export default async function UnifiedHealthIntelligence({
  params,
}: {
  params: any;
}) {
  ...
  const fetchUHIEData = async () => {
    try {
      const pid = (await params).id.replace("P", "");
    ...
    <div className="py-6 space-y-6">
      ...
      <div className="flex justify-between items-end">
        ...
        <button
          ...
          disabled={isFusing}
          className="flex items-center gap-2 px-5 py-2.5 bg-indigo-600 hover:bg-indigo-500 text-white font-semibold rounded-xl transition-all shadow-[0_0_20px_rgba(79,70,229,0.3)] disabled:opacity-50"
        >
          {" "}
          {isFusing ? (
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {" "}
        {}{" "}
        <div className="bg-gradient-to-br from-slate-900 to-indigo-900/20 border border-indigo-500/30 rounded-2xl p-6 relative overflow-hidden">
          {" "}
        ...
        {}{" "}
        <div className="dark bg-slate-900/50 border border-slate-800 rounded-2xl p-6 relative">
          {" "}
        ...
        <div className="dark bg-slate-900/50 border border-slate-800 rounded-2xl p-6">
          ...
          <div className="space-y-3">
            ...
            <div className="dark w-full bg-slate-800 rounded-full h-1.5">
              {" "}
              <div
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {" "}
        {}{" "}
        <div className="dark bg-slate-900/50 border border-slate-800 rounded-2xl p-6 min-h-[400px] flex flex-col">
          {" "}
          ...
          <div className="flex-1 bg-slate-950/50 rounded-xl border border-slate-800/80 flex items-center justify-center relative overflow-hidden">
            {" "}
            {}{" "}
            <div
          ...
          <div className="dark bg-slate-900/50 border border-slate-800 rounded-2xl p-6">
            ...
            <div className="space-y-3">
              ...
              {correlations.map((c) => (
                <div
                  key={c.id}
                  className="dark p-4 bg-slate-800/40 rounded-xl border border-slate-700/50 hover:bg-slate-800 transition-colors"
                >
                  {" "}
                  <div className="flex items-center gap-3 mb-2">
                    {" "}
                    <span className="dark px-2 py-1 bg-slate-900 rounded text-xs text-muted-foreground font-medium">
                      {c.source_factor}
                    </span>{" "}
          ...
          <div className="dark bg-slate-900/50 border border-slate-800 rounded-2xl p-6">
            ...
            <div className="space-y-3">
              ...
              {events.map((e) => (
                <div
                  key={e.id}
                  className="dark flex gap-4 p-3 border-l-2 border-cyan-500 bg-slate-800/20"
                >
                  {" "}
                  <div>
                    {" "}
