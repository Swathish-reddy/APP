...
export default function PatientProfileLayout({
  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#030712]">
      {" "}
      {}{" "}
      <div className="sticky top-0 z-40 w-full backdrop-blur-xl bg-[#030712]/80 border-b border-slate-800">
        {" "}
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex items-center h-16 gap-4">
            ...
            <Link
              href="/patients"
              className="p-2 rounded-full hover:bg-slate-800 text-muted-foreground hover:text-slate-200 transition-colors"
            >
              {" "}
              ...
              {tabs.map((tab) => {
                ...
                return (
                  <Link
                    key={tab.name}
                    href={tab.href}
                    className={`relative whitespace-nowrap px-4 py-2.5 rounded-t-xl text-sm font-medium flex items-center gap-2 transition-colors ${isActive ? "text-cyan-400 bg-slate-800/50" : "text-muted-foreground hover:text-slate-200 hover:bg-slate-800/30"}`}
                  >
                    {" "}
