...
export default function PatientMedications({ params, patientId: propPatientId }: { params?: any, patientId?: string }) {
  ...
  if (!medData)
    return (
      <div className="flex flex-col items-center justify-center h-[80vh] text-center">
        ...
        <h2 className="text-3xl font-extrabold text-white mb-2">No Medication Data</h2>
  ...
  return (
    ...
    >
      {/* Header Section */}
      <motion.div variants={itemVariants} className={`flex flex-col md:flex-row items-start md:items-center justify-between gap-4 bg-slate-800/80 border ${isHighRisk ? 'border-rose-500/50' : 'border-indigo-500/50'} p-6 rounded-3xl backdrop-blur-xl shadow-2xl relative overflow-hidden`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <motion.div variants={itemVariants} className="lg:col-span-8 space-y-6">
          ...
          <div className="bg-slate-800/80 border border-slate-700/80 rounded-3xl p-8 backdrop-blur-md relative overflow-hidden shadow-2xl">
            ...
            <div className="space-y-6">
              {/* Drug Interactions */}
              <div>
                <h4 className="text-sm font-black text-slate-300 uppercase tracking-widest mb-3 drop-shadow-sm">Critical Interactions</h4>
              ...
              <div>
                <h4 className="text-sm font-black text-slate-300 uppercase tracking-widest mb-3 drop-shadow-sm">Renal & Hepatic Contraindications</h4>
                {Array.isArray(medData["5_allergy_contraindication_review"]) ? (
                  ...
                  <div className="bg-slate-700/60 border border-slate-600/50 p-5 rounded-2xl flex items-start gap-4 shadow-inner">
                    <p className="text-slate-200 font-bold">{medData["5_allergy_contraindication_review"]}</p>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="bg-slate-800/80 border border-indigo-500/30 rounded-3xl p-6 shadow-xl shadow-indigo-900/20">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <p className="text-white font-bold text-base leading-relaxed mt-1">{medData["7_medication_effectiveness"]}</p>
                </div>
                <div>
                  <span className="text-xs text-indigo-200 font-bold uppercase tracking-wider">Lab Correlation</span>
                  <p className="text-white font-bold text-base leading-relaxed mt-1">{medData["8_laboratory_correlation"]}</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-slate-800/80 border border-teal-500/30 rounded-3xl p-6 shadow-xl shadow-teal-900/20">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <p className="text-white font-bold text-base leading-relaxed mt-1">{medData["11_predictive_analytics"]}</p>
                </div>
                <div>
                  <span className="text-xs text-teal-200 font-bold uppercase tracking-wider">Adherence Profile</span>
                  <p className="text-white font-bold text-base leading-relaxed mt-1">{medData["9_adherence_analysis"]}</p>
                </div>
              </div>
            </div>
          </div>
