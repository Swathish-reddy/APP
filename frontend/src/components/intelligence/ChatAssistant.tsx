...
import { Bot, User, Send, Loader2, Stethoscope, ChevronRight, Mic, MicOff } from "lucide-react";
...
const MarkdownRenderer = ({ content }: { content: string }) => {
  const createMarkup = (text: string) => {
    let html = text;
    // Code blocks
    html = html.replace(/```([\s\S]*?)```/g, '<pre class="bg-background/50 p-3 rounded-lg overflow-x-auto my-2 border border-white/10 text-xs font-mono text-emerald-200">$1</pre>');
    // Inline code
    ...
    html = html.replace(/^# (.*$)/gim, '<h1 class="text-2xl font-black mt-6 mb-3 text-foreground">$1</h1>');
    // Bullet lists
  ...
  return (
    <div 
      className="text-sm leading-relaxed prose-sm max-w-none text-foreground"
...
export const ChatAssistant: React.FC<ChatAssistantProps> = ({
  ...
  const sendMessage = async (text: string) => {
    ...
    try {
      ...
      const aiMsg: Message = {
        role: "assistant",
        content: data.reply || "I could not process that query. Please try again.",
        timestamp: new Date().toLocaleTimeString(),
      };
  ...
  return (
    <Card className="h-full border-none shadow-none flex flex-col bg-background rounded-none md:rounded-3xl overflow-hidden relative">
      {}
      <div className="absolute top-0 right-0 w-full md:w-full md:w-96 h-96 bg-indigo-500/10 blur-[100px] pointer-events-none rounded-full"></div>
      <div className="absolute bottom-0 left-0 w-full md:w-full md:w-96 h-96 bg-emerald-500/10 blur-[100px] pointer-events-none rounded-full"></div>
      
      <CardHeader className="px-5 py-4 border-b border-white/10 bg-background/20 backdrop-blur-xl z-10">
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-5 space-y-6 z-10 scroll-smooth">
          ...
          >
            <div
              className={`w-8 h-8 rounded-full flex-shrink-0 flex items-center justify-center shadow-lg ${
                msg.role === "assistant" 
                  ? "bg-gradient-to-br from-indigo-500 to-purple-600 text-foreground" 
            ...
            >
              <div
                className={`px-4 py-3 text-sm shadow-xl border ${
                  msg.role === "assistant" 
                    ? "bg-card/80 border-border/50 rounded-2xl rounded-tl-sm backdrop-blur-md" 
                    : "bg-gradient-to-br from-emerald-600 to-teal-700 border-emerald-500/30 text-foreground rounded-2xl rounded-tr-sm"
                }`}
              >
                 {msg.role === "assistant" ? (
        ...
        {loading && (
          <div className="flex gap-4 w-full">
            <div className="w-8 h-8 rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 text-foreground flex items-center justify-center flex-shrink-0 shadow-lg shadow-indigo-500/20">
              <Bot className="w-4 h-4 drop-shadow-md" />
            </div>
            <div className="bg-card/80 border border-border/50 rounded-2xl rounded-tl-sm px-5 py-4 flex items-center gap-3 backdrop-blur-md shadow-xl">
              <Loader2 className="w-4 h-4 animate-spin text-indigo-400" />
              <span className="text-sm font-bold text-foreground tracking-wide animate-pulse">
                Thinking...
              </span>
            </div>
          </div>
        ...
        <div className="px-5 pb-3 flex flex-wrap gap-2 z-10">
          {STARTER_QUESTIONS.map((q, i) => (
            <button
              ...
              className="text-[11px] font-bold bg-muted/50 hover:bg-indigo-500/20 border border-border/50 hover:border-indigo-500/40 text-foreground hover:text-indigo-200 rounded-full px-3 py-1.5 transition-all shadow-sm flex items-center gap-1 group"
            >
      ...
      <div className="p-4 border-t border-white/10 bg-background/20 backdrop-blur-xl flex gap-3 z-10">
        <Button
          onClick={toggleListening}
          variant="outline"
          className={`h-11 w-11 p-0 rounded-xl transition-all ${isListening ? 'bg-red-500 hover:bg-red-600 text-white border-transparent shadow-lg shadow-red-500/25 animate-pulse' : 'bg-card/50 border-border text-foreground hover:bg-muted'}`}
        ...
        <Input
          ...
          className="flex-1 bg-card/50 border-border text-foreground placeholder:text-muted-foreground focus-visible:ring-indigo-500/50 h-11 px-4 rounded-xl shadow-inner"
          disabled={loading}
        />
        <Button
          onClick={() => sendMessage(input)}
          disabled={loading || !input.trim()}
          className="h-11 w-11 p-0 rounded-xl bg-gradient-to-br from-indigo-500 to-purple-600 hover:from-indigo-400 hover:to-purple-500 text-foreground shadow-lg shadow-indigo-500/25 transition-all active:scale-95"
        >
