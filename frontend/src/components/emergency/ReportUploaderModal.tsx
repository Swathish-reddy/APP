        ...
        >
          ...
          {!uploading && !complete ? (
            <div 
              ...
              onDrop={handleDrop}
              className={`border-2 border-dashed rounded-xl p-10 flex flex-col items-center justify-center text-center transition-colors ${dragActive ? 'border-blue-500 bg-blue-500/10' : 'border-slate-700 bg-slate-800/50 hover:bg-slate-800'}`}
            >
              <UploadCloud className={`w-12 h-12 mb-4 ${dragActive ? 'text-blue-400 animate-bounce' : 'text-slate-500'}`} />
              <p className="text-white font-medium mb-1">Drag and drop file here</p>
              <p className="text-xs text-slate-500 mb-4">Supported formats: PDF, DICOM, JPG, PNG (Max 50MB)</p>
              <button onClick={simulateUpload} className="px-4 py-2 bg-slate-700 hover:bg-slate-600 text-white text-sm font-medium rounded-lg transition-colors">
                Browse Files
              </button>
            </div>
          ) : (
            <div className="border border-slate-700 bg-slate-800/50 rounded-xl p-8 flex flex-col items-center justify-center text-center">
              {complete ? (
                <>
                  ...
                  <p className="text-sm text-slate-400">Updating triage plan...</p>
                </>
              ) : (
                <>
                  ...
                  <p className="text-xs text-slate-400 text-right w-full">{progress}%</p>
                </>
              )}
            </div>
          )}
