...
export default function RightPanelCapacity() {
  ...
  if (!capacity) {
    return (
      <div className="h-full flex flex-col bg-slate-900/60 border border-slate-800/80 rounded-2xl backdrop-blur-md overflow-hidden p-4">
        <div className="animate-pulse space-y-4">
          <div className="h-10 bg-slate-800/50 rounded w-1/2"></div>
    ...
    <div className="h-full flex flex-col bg-slate-900/60 border border-slate-800/80 rounded-2xl backdrop-blur-md overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 scrollbar-hide">
        ...
        <div className="space-y-3">
          <h3 className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Critical Beds</h3>
          ...
          <div className="bg-slate-800/40 p-3 rounded-xl border border-slate-700/50">
            <div className="flex justify-between items-center">
              ...
              <div className="text-sm">
                <span className="font-bold text-white">{capacity.icu_beds.occupied}</span>
                <span className="text-slate-500"> / {capacity.icu_beds.total}</span>
              </div>
            </div>
          ...
          <div className="bg-slate-800/40 p-3 rounded-xl border border-slate-700/50">
            <div className="flex justify-between items-center">
              ...
              <div className="text-sm">
                <span className="font-bold text-white">{capacity.er_beds.occupied}</span>
                <span className="text-slate-500"> / {capacity.er_beds.total}</span>
              </div>
            </div>
        ...
        <div className="space-y-3 pt-2">
          <h3 className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Equipment & Resources</h3>
          
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-slate-800/40 p-3 rounded-xl border border-slate-700/50 flex flex-col items-center justify-center text-center">
              <Wind className="w-5 h-5 text-blue-400 mb-1" />
              <span className="text-xs text-slate-400">Ventilators</span>
              <span className="text-lg font-bold text-white">{capacity.ventilators.total - capacity.ventilators.occupied} <span className="text-xs font-normal text-slate-500">Avail</span></span>
            </div>
            <div className="bg-slate-800/40 p-3 rounded-xl border border-slate-700/50 flex flex-col items-center justify-center text-center">
              <Droplets className="w-5 h-5 text-red-500 mb-1" />
              <span className="text-xs text-slate-400">O- Blood</span>
              <span className="text-lg font-bold text-white">{capacity.blood_units.o_neg} <span className="text-xs font-normal text-slate-500">Units</span></span>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-3 pt-2">
          ...
          <div className="space-y-2">
            {capacity.on_call_specialists.map((doc: any, i: number) => (
              <div key={i} className="flex items-center justify-between bg-slate-800/40 p-2.5 rounded-lg border border-slate-700/30">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-slate-400 uppercase">{doc.specialty}</p>
                  </div>
                </div>
